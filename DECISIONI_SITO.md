...
- **Contenuti nuovi, per livello.** A1: 87% di acqua, oltre 200 sostanze, 30 ml di colostro il primo giorno contro i 5 ml che tiene lo stomaco del neonato. A2: melatonina di notte e cortisolo di giorno; il latte per un bambino prematuro contiene più proteine, grassi e difese per alcune settimane. B1: gli oligosaccaridi, più di 200 tipi che il bambino non digerisce, nutrono il _Bifidobacterium infantis_ e funzionano da esche per virus e batteri. B2: i globuli bianchi passano dallo 0-2% fino al 94% delle cellule durante un'infezione della madre **o del solo bambino**, e tornano al livello di partenza con la guarigione; gli anticorpi arrivano dall'intestino della madre alla ghiandola mammaria. C1: la lattoferrina difende sottraendo ferro ai batteri; il latte non è sterile; la formula artificiale è «una fotografia», il latte materno «una conversazione tra due corpi».
...
## 2026-09-02 - Lettura «La storia della mafia in Italia» in tutte e 9 le lingue

- **Perché.** Domanda ricorrente degli studenti stranieri: com'è nata la mafia, gli obiettivi iniziali erano buoni, quante mafie esistono, si è espansa all'estero, è vero che gli italiani la considerano buona e lo Stato cattivo. La lettura risponde con dati e fonti, non con impressioni.
- **La tesi della pagina è documentata, non morale.** La mafia non nasce dalla miseria ma dove il valore era alto e ricattabile: lo studio del 2017 sull'inchiesta Damiani (1881-1886) trova presenza mafiosa marcata in 51 comuni siciliani su 143, concentrata nelle zone dei limoni, che rendevano circa sessanta volte la media dell'isola. La formula di Franchetti del 1876 - «industria della violenza» - spiega il meccanismo: si vende protezione contro un pericolo prodotto da chi la vende.
- **La domanda «mafia buona, Stato cattivo» ha una scheda dedicata.** I sondaggi non dicono che gli italiani ammirino i clan (50% le mette fra i problemi più urgenti, 73% le descrive come organizzazioni di manager); dicono che l'83% ritiene che la politica le abbia favorite. Sfiducia nelle istituzioni e ammirazione per la mafia sono due cose diverse, e la pagina lo scrive esplicitamente. Dove lo Stato è assente resta il consenso passivo: dipendenza materiale, non adesione.
- **Quattro mafie, non una.** Camorra (documento del 1735), Cosa Nostra, 'ndrangheta (oggi la più solida, per i legami di sangue), Sacra Corona Unita (carcere di Bari, 1° maggio 1983). L'espansione all'estero è documentata con l'operazione Eureka del maggio 2023 (otto paesi, oltre cento arresti, tre tonnellate di cocaina) e i 48 «locali» calabresi nel Centro-Nord censiti dalla DIA nel 2024.
- **Fonti in fondo alla pagina**, con link: «The Journal of Economic History» 2017, Treccani, Ipsos 2026, DIA 2024, ANSA, Libera.
- **Nove pagine + 54 PDF.** Italiano più le 8 lingue, con gli slug localizzati già previsti dagli hreflang; tessera aggiunta ai 9 indici delle letture e 9 voci nella sitemap. Audit di lingua, sito, link, vocabolario, grammatica e collegamenti tutti verdi.
- **L'immagine è ancora provvisoria.** La quota giornaliera di generazione è esaurita: `public/assets/reading-storia-mafia-italia.webp` e `-card.webp` restano un fondo di carta invecchiata. Il prompt definitivo e la procedura di sostituzione sono in `docs/prompt-immagine-storia-mafia.md`.
- **Parità di migrazione invariata**: 490 differenze su 1119 pagine, più 9 pagine «extra in dist» - le nove di questa lettura, che in `legacy-html/` non esistono.

## 2026-09-03 - Generazione immagini via OpenRouter, immagine definitiva della mafia

- **Nuovo canale di generazione**: `scripts/generate-image.mjs`, che chiama l'API immagini di OpenRouter (`POST /api/v1/images`) invece del server MCP Hugging Face. La chiave sta in `.env` (`OPENROUTER_API_KEY`, escluso da git), non passa mai in chiaro nella chat né nei log dello script.
- **Perché**: la quota gratuita di Hugging Face (ZeroGPU) è per account, si azzera ogni 24 ore e basta per 5-8 immagini al giorno; i crediti prepagati funzionano solo con l'abbonamento PRO. OpenRouter non ha quota giornaliera: si paga a consumo, senza abbonamento fisso.
- **Modello scelto**: `google/gemini-3-pro-image` (Nano Banana Pro), risoluzione 2K, aspect ratio 16:9. Costo per immagine: **$0.13** circa (un'illustrazione ne genera due file, hero 960×540 e card 640×360, dalla stessa generazione).
- **Immagine definitiva della lettura sulla mafia** generata e pubblicata: sostituisce il fondo di carta invecchiata provvisorio. Stesso stile della lettura sul caffè - collage seppia, oggetti fotorealistici (limoni, registro, chiave, coppola, giornale) e incisione ottocentesca sul muro - generato con il prompt già pronto in `docs/prompt-immagine-storia-mafia.md`.
- **PDF rigenerati**: 54 file (`storia-della-mafia-in-italia`, tutte le lingue e i livelli).
- **Prossimo uso previsto**: le 64 illustrazioni mancanti del vocabolario del cibo (vedi `docs/prompt-immagini-vocabolario.md`), oggi bloccate dalla quota gratuita.

## 2026-09-03 - Il cibo arriva a 72 parole: le 56 immagini mancanti generate

- **Perché**: dal 2026-08-31 la scheda di vocabolario "Il cibo" aveva 56 parole pronte ma senza immagine (`scripts/data/food-vocabulary-extra.mjs`), bloccate dalla quota gratuita di Hugging Face. Con il canale OpenRouter aperto lo stesso giorno per l'immagine della lettura sulla mafia, si sono generate tutte in un colpo solo.
- **Modello scelto per queste immagini: `openai/gpt-image-1-mini`, non `google/gemini-3-pro-image`.** Testato prima su soggetti di difficoltà crescente (mela, funghi multipli, bottiglia di vetro trasparente con liquido) a `quality: "low"`: risultato indistinguibile a occhio dalla qualità superiore una volta ridotto a 512×512, come già sono tutte le immagini del vocabolario. Costo: **$0.1284 per 53 immagini** (le prime 3 di test sono costate $0.0073 a parte), contro gli $0.13 di una sola immagine con il modello usato per le letture. La scelta del modello dipende dal soggetto: fotografie di prodotto semplici e isolate su sfondo bianco vogliono il modello economico, scene narrative complesse (collage storici) il modello di punta.
- **Nuovi script**: `scripts/generate-vocabulary-images.mjs` (chiama OpenRouter parola per parola, salta quelle già pubblicate, supporta `--only`, `--limit`, `--dry-run`) e i dati in `scripts/data/food-vocabulary-image-prompts.mjs` (soggetto in inglese per slug, parallelo a `food-vocabulary-extra.mjs`). Il flusso resta lo stesso di prima: `expand-food-vocabulary.mjs --images <cartella>` converte e inserisce nelle 9 pagine.
- **Risultato**: 16 → 72 parole in tutte e 9 le lingue. Audit di lingua, sito, link, vocabolario e grammatica tutti verdi; parità di migrazione invariata (490 differenze, 9 extra in dist - `vocabolario/cibo.html` era già nel conteggio dal 2026-09-01).
- **`docs/prompt-immagini-vocabolario.md` aggiornato**: tutte le 64 righe segnate ✅, nuova sezione 5 con il procedimento OpenRouter da riusare per le prossime parole.

## 2026-09-03 - Le 56 immagini del cibo avevano lo sfondo sbagliato: corretto

- **Il problema**: segnalato dall'utente su «la torta» - un rettangolo grigio chiaro visibile intorno al soggetto, non fuso con lo sfondo bianco della scheda. Causa: `openai/gpt-image-1-mini` non genera mai un bianco puro (#FFFFFF) ma qualcosa come #F9F7F7, e la pipeline incollava l'immagine come quadrato opaco senza un vero ritaglio - a differenza delle prime 8 immagini del vocabolario, che hanno trasparenza reale.
- **Bianco o trasparente? Trasparente, quando è possibile farlo bene.** Uno sfondo bianco "quasi giusto" crea sempre un bordo visibile perché non può mai combaciare esattamente con `.word-card img { background: #fff }`; un vero ritaglio trasparente invece si fonde sempre, qualunque sfumatura genera il modello.
- **Nuovo script `scripts/remove-white-background.py`**: flood fill dai bordi dell'immagine verso l'interno, confrontato con un colore di sfondo stimato una volta sola (mediana del perimetro), non con il pixel vicino via via incontrato - un primo tentativo con soglia "a catena" attraversava i gradienti morbidi (il riflesso di una torta) e mangiava il soggetto.
- **Otto immagini non si possono ritagliare col colore**: quando il soggetto stesso è bianco o quasi (farina, sale, zucchero, mozzarella, yogurt in vasetto bianco, un bicchiere d'acqua o di latte, un bicchiere di vino dove il vetro è trasparente) non c'è abbastanza differenza di colore per distinguere soggetto da sfondo: il flood fill toglieva pezzi veri (metà di una mozzarella, il bordo di una ciotola). Per queste nove (le otto più «il gelato», un difetto minore sullo stesso bordo) si è usata una correzione di colore invece del ritaglio: una scala lineare per canale che sposta lo sfondo stimato esattamente a bianco puro, senza decidere pixel per pixel che cosa è sfondo - non può creare buchi perché non ritaglia nulla.
- **Verifica**: le 56 immagini riviste una per una (foglio di contatto), le nove corrette a mano confrontate prima e dopo. Rigenerati i file in `public/assets/vocabolario/`, nessun nuovo costo (riusate le immagini grezze già generate). Build e tutti gli audit verdi.

## 2026-09-03 - Nuova lezione di vocabolario «Il mare», 30 parole in 9 lingue

- **Perché questa e non un'altra**: l'indice del vocabolario annunciava da tempo tre lezioni «In preparazione» (mare, montagna, città) come segnaposto senza pagina dietro. Erano una promessa fatta al visitatore: si comincia mantenendola.
- **30 parole** scelte fra oggetti riconoscibili a colpo d'occhio (conchiglia, ombrellone, salvagente, faro, granchio, medusa...), evitando sovrapposizioni con le schede già esistenti: niente «il pesce» (è nel cibo), niente «gli occhiali da sole» o «il costume» (sono nell'abbigliamento). Sei parole di ambiente (spiaggia, isola, molo, scoglio) sono state generate come piccole scene isolate, non come panorami, per non rompere lo stile delle altre immagini.
- **Le pagine nascono dalla lezione della cucina già tradotta**, non da zero: `scripts/create-sea-vocabulary.mjs` legge la pagina della cucina in ognuna delle 9 lingue e ne sostituisce solo titolo, testata, parole e contatori. Tutte le etichette di servizio («Riconosci la parola», «Frasi da tradurre», i bottoni, la barra di avanzamento, la nota finale) restano quelle già tradotte: non si reinventano e non si sbagliano. Stesso principio di `expand-food-vocabulary.mjs`.
- **I segnaposto diventano schede vere**: lo script sostituisce il riquadro «In preparazione» del mare con il collegamento vero nei 9 indici, copiando l'etichetta di stato («Lezione disponibile», «Lesson available»...) da una scheda già presente. Restano i due segnaposto di montagna e città.
- **Due modelli diversi per le immagini, come da decisione del 2026-09-03**: le 30 foto delle parole con `openai/gpt-image-1-mini` a qualità «low» ($0.0722 in tutto), la **testata** con `google/gemini-3-pro-image` ($0.13), perché è l'immagine che si vede per prima e definisce la lezione. La prima testata generata era un cartoon con contorni neri marcati, fuori stile rispetto alle altre: rifatta con un prompt che chiede esplicitamente illustrazione pittorica morbida senza contorni.
...
## 2026-09-04 - Lettura «L'insetto con gli ingranaggi» in tutte e 9 le lingue

- **Perché.** Richiesta di Martin, partendo da un articolo di «Svegliatevi!» (agosto 2015) sull'_Issus coleoptratus_. Il criterio resta quello di sempre: si insegna italiano usando un argomento che vale la pena leggere anche se non si studia niente. Qui l'argomento è un fatto verificabile e sorprendente — il primo ingranaggio meccanico funzionante trovato in un essere vivente.
- **I nove fatti stanno in tutti e cinque i livelli**, come vuole la regola del 2026-08-31: nome e taglia dell'insetto, i due ingranaggi alla base delle zampe posteriori della ninfa, la striscia di circa 400 micrometri con 10-12 denti, i 30 microsecondi di scarto fra le due zampe, i 3,9 metri al secondo raggiunti in 2 millesimi di secondo (quasi 200 g), il perché il sistema nervoso è troppo lento, la base arrotondata dei denti come negli ingranaggi d'acciaio, la scomparsa del meccanismo dopo l'ultima muta, e la scoperta di Burrows e Sutton su «Science» il 13 settembre 2013.
- **Due approfondimenti e due LAB di lingua.** «Perché il cervello non basta» spiega dove nasce davvero il ritardo nervoso (non la distanza, i passaggi fra le cellule) e perché togliere un'alternativa è più affidabile che dare un ordine; «Perché da adulto li perde» presenta le due ipotesi degli autori dicendo esplicitamente che nessuna è dimostrata. I due LAB sono lezioni di italiano vere: il lessico degli ingranaggi (_ingranare_, _la marcia_, i prefissi _micro-_ e _milli-_, i nomi ricavati dai verbi) e il modo di dire i numeri piccolissimi (virgola invece del punto, ordinali per le frazioni, _metri al secondo_, il moltiplicatore prima di _volte_).
- **Fonti in fondo alla pagina**, con link: «Science» 2013, University of Cambridge, «Journal of Experimental Biology», Smithsonian Magazine e l'articolo di «Svegliatevi!» da cui è partita la richiesta.
- **Una lettura nuova non si scrive nove volte.** `scripts/create-issus-reading.mjs` fa per le letture quello che `create-sea-vocabulary.mjs` fa per il vocabolario: testo italiano in `scripts/data/issus-reading-it.mjs`, cornice localizzata in `scripts/data/issus-reading-i18n.mjs`, e il resto clonato da risorse già revisionate — i metadati e la call to action dalla lettura sulla mafia, la tessera «Scienza» da quella sul sonar del delfino. Così nav, footer, etichette dei pulsanti e testi di servizio restano quelli già corretti in ogni lingua.
- **Immagine di testata con `google/gemini-3-pro-image`** ($0.1348), come da decisione del 2026-09-03 per le testate. Mostra insieme le due cose chieste: la ninfa su una foglia d'edera e, ingrandita in un tondo, la coppia di strisce dentate che si incastrano, in tonalità ambrate di chitina e non di metallo. Stile allineato alle altre letture scientifiche (api, DNA): macro fotorealistica, luce calda, nessun contorno nero. Prompt e vincoli in `docs/prompt-immagine-insetto-ingranaggi.md`.
- **Corretta una spaziatura sbagliata nei PDF.** `node_text()` in `scripts/generate-pdfs.py` univa i pezzi di testo con uno spazio, quindi `<em>Issus coleoptratus</em>.` diventava «Issus coleoptratus .» e ogni `<strong>` dei LAB si portava dietro uno spazio di troppo. Ora unisce senza spazio: la spaziatura giusta è già nell'HTML. Riguarda tutti i PDF, ma per ora sono rigenerati solo i 54 di questa lettura; gli altri si allineano quando si rigenera tutto.
- **Nove pagine + 54 PDF**, tessera aggiunta ai 9 indici delle letture nella sezione Scienza e 9 voci nella sitemap. Audit di lingua, sito, link, vocabolario e grammatica tutti verdi.
- **Parità di migrazione**: 490 differenze su 1119 pagine (invariata), **27 pagine «extra in dist»** — le 9 della mafia, le 9 della lezione «Il mare» (mai registrate il 2026-09-03) e le 9 di questa lettura. La soglia in `AGENTS.md` è stata aggiornata.

## 2026-09-07 - Le parole dei brani diventano cliccabili: glossario, esempio e pronuncia

- **La richiesta di Martin**: rendere cliccabili le parole di «Il cane e l'osso» per mostrare esempi, immagini e pronuncia, oppure dare la traduzione del brano a parte.
- **Scelta: glossario cliccabile, non traduzione affiancata.** Una traduzione a fianco fa leggere quella e non l'italiano, e a B1-C1 toglie proprio l'esercizio che quei livelli devono fare. Il glossario serve invece a ogni riga. La traduzione integrale resta possibile più avanti, ma nascosta in un `<details>` chiuso e solo per A1 e A2, come le glosse `.q-gloss`.
- **Le parole non sono marcate nell'HTML.** `public/assets/glossario/<id>.json` elenca le voci, `public/assets/glossario.js` le trova a runtime dentro `.story-text`. Marcarle a mano avrebbe voluto dire 9 lingue x 5 livelli di modifiche, HTML raddoppiato, PDF sporcati e ogni correzione futura da fare 45 volte. Così il brano resta **identico** nelle 9 lingue (verificato: il testo del primo paragrafo non cambia di un carattere), i PDF non cambiano e gli audit non vedono niente di nuovo.
- **Al clic**: la parola con l'articolo, la pronuncia italiana (`speechSynthesis` a 0.85, la stessa delle schede di vocabolario, quindi nessun file audio da produrre), la traduzione nella lingua del visitatore e una frase d'esempio **sempre in italiano**, secondo `REGOLE_LINGUE.md`. Sulla pagina italiana la «traduzione» è una definizione italiana e l'etichetta diventa «Significato».
- **Ogni voce si segna una volta sola per livello**, non a ogni occorrenza: con 63 voci vengono 97 parole sottolineate su cinque livelli, e il testo resta leggibile invece di diventare un tappeto di sottolineature.
- **La stessa parola non ha sempre lo stesso significato** (osservazione di Martin). La glossa dà il senso che la parola ha _in questo testo_: «i limiti» in C1 sono i difetti, non il confine. Quando i sensi sono due nello stesso brano si scrivono due voci, distinte da `contesto` (una parola vicina nello stesso paragrafo) o da `livelli`: «superficie» è il pelo dell'acqua nel primo paragrafo del C1 e «solo da fuori» nell'ultimo, e la pagina le distingue davvero. Dove il senso ovvio sarebbe quello sbagliato lo si dice dentro la traduzione: `apparentemente` = «seemingly, only in appearance (NOT “apparently”)», `restituire` = rimandare indietro un'immagine e non ridare un oggetto, `il bene` è un nome e non l'avverbio, `reale` non è «del re», `la corrente` non è l'elettricità.
- **Nuovo script riusabile** `scripts/attach-glossary.mjs <id> <pagine...>`: mette CSS e script in `extraHead` e `bodyScripts` con il prefisso giusto per la profondità di ogni pagina. Idempotente: rilanciarlo aggiorna la versione invece di aggiungere una seconda copia.
- **Verifica**: provate nel browser sulla build reale la pagina italiana, l'inglese, la ceca e la giapponese (etichette tradotte, esempio in italiano, pronuncia presente); i due sensi di «superficie» danno due riquadri diversi; «bene» compare solo nel C1 e solo nella frase giusta. Chiusura con Esc, con un clic fuori e ricliccando la parola. Su telefono (375x812) il riquadro diventa una scheda in basso. Corretti due difetti trovati provando: il focus sul pulsante di chiusura faceva saltare la pagina, e il riquadro poteva finire sotto l'header fisso.
- **Audit** di lingua, sito, link, vocabolario e grammatica verdi; `npm run check` pulito a parte i 4 file già fuori formato da prima. **Parità di migrazione invariata**: 490 differenze su 1119 pagine, 27 pagine extra.
- **Restano da fare**: le immagini per una decina di parole concrete (osso, ponte, fiume, macelleria, denti, bottega, sagoma, fauci) con `openai/gpt-image-1-mini`, circa 2 centesimi; la traduzione integrale nascosta per A1 e A2; il glossario sulle altre favole e letture.

## 2026-09-07 - Lettura «Proteine: quante ne servono davvero» in tutte e 9 le lingue

- **Perché.** Richiesta di Martin: una lettura sulle proteine e la prestazione fisica e mentale, che rispondesse a domande precise — quante proteine servono, di che tipo, animali o vegetali, se cereali e legumi vanno combinati, che cosa cambia con lo sport, vegetariano o cheto, i miti, i consigli pratici e come spendere poco. È il criterio di sempre: si insegna italiano su un argomento che vale la pena leggere anche senza studiare.
- **Tredici fatti in tutti e cinque i livelli**, come vuole la regola del 2026-08-31: la definizione (20 amminoacidi, 9 essenziali), 0,83 g/kg secondo l'EFSA e 0,90 secondo i LARN italiani, 1,2-2,0 g/kg per chi si allena con il tetto misurato a 1,6 (meta-analisi Morton 2018, 49 studi, 1.863 persone), 1,0-1,2 g/kg dopo i 65 anni per la resistenza anabolica, la distribuzione a ~0,4 g/kg per pasto (Areta 2013), gli amminoacidi limitanti (metionina nei legumi, lisina nei cereali), la complementarietà cereale + legume **senza** obbligo dello stesso pasto, il +10-20% e la B12 obbligatoria per i vegani, la cheto (primi chili idrici, nessun vantaggio a 12 mesi, prestazione peggiore ad alta intensità), gli esempi di atleti veri, il cervello (20% dell'energia, ~120 g di glucosio al giorno, triptofano e tirosina), il mito della finestra dei 30 minuti e il costo di dieci grammi di proteine.
- **Gli esempi con nomi veri sono stati aggiunti su richiesta di Martin durante la sessione**, ed è la decisione di metodo più utile: un articolo di nutrizione senza casi concreti resta astratto, ma i casi da soli sono aneddoti. La lettura li dà da entrambe le parti — Carl Lewis vegano dal 1990 e 9,86 s a Tokyo nel 1991, Scott Jurek vegano dal 1999 con sette Western States, Patrik Baboumian e i 555 kg del primato Guinness 2013, Venus Williams dal 2011, Lewis Hamilton dal 2017; e sul fronte opposto Zach Bitter, 100 miglia in 11h19'13" nel 2019 a carboidrati bassissimi, più lo studio FASTER di Volek (2016: 1,54 contro 0,67 grammi di grassi bruciati al minuto) — e poi dice esplicitamente che cosa dimostrano e che cosa no, citando anche il conflitto di interessi del documentario «The Game Changers». Tutti i dati sono stati verificati uno per uno prima di scriverli.
- **Struttura più ricca del modello**: oltre ai tre approfondimenti con parole e domande (animali o vegetali; ogni sport ha il suo piatto; vegetariano o cheto), tre schede di solo testo con badge nuovi — **ESEMPI** (gli atleti veri), **MITI** (otto miti smontati uno per uno) e **SPESA** (il prezzo di dieci grammi di proteine da sei fonti diverse, sei regole pratiche e una giornata da 95 grammi di proteine per poco più di quattro euro) — più i due LAB di lingua: il lessico del fabbisogno (_fabbisogno_, _assumere_ nei suoi tre sensi, _apporto_, _carenza_, _porzione_/_dose_/_razione_, _magro_) e il modo di dire le quantità (_per_ contro _al giorno_, la virgola decimale, l'articolo davanti alle percentuali, _una sessantina di_, _di più_ contro _in più_).
- **Dodici fonti in fondo alla pagina**, con link: EFSA 2012, LARN/SINU 2014, Morton 2018, la posizione congiunta Academy of Nutrition and Dietetics/Dietitians of Canada/ACSM 2016, la posizione sulle diete vegetariane 2016, FAO 2013 (DIAAS), Areta 2013, Burke 2017, Trommelen 2023, PROT-AGE 2013, lo studio FASTER 2016 e il primato Guinness di Baboumian.
- **Nuovo script `scripts/create-proteine-reading.mjs`**, sullo stampo di `create-issus-reading.mjs`: testo italiano in `scripts/data/proteine-reading-it.mjs`, cornice localizzata in `scripts/data/proteine-reading-i18n.mjs`, e metadati, call to action e tessera «Scienza» clonati dalla lettura sull'insetto con gli ingranaggi, già revisionata in tutte e 9 le lingue.
- **Immagine di testata con `google/gemini-3-pro-image`** ($0,1349), come da decisione del 2026-09-03. **Su richiesta esplicita di Martin non rappresenta carne, per rispetto di chi legge ed è vegano**: la scelta è stata estesa anche a pesce, uova e latticini, così la testata mostra solo fonti vegetali — legumi, cereali, semi, frutta secca, tofu, edamame — che sono poi anche quelle economiche di cui parla la scheda «Spesa». Prompt e vincoli in `docs/prompt-immagine-proteine.md`.
- **Breadcrumb giapponese**: la nuova pagina usa 読解 («lettura/comprensione»), non 測定値 («valori misurati») come le altre letture in giapponese. 測定値 è una traduzione sbagliata ereditata dalla migrazione: resta da correggere sulle altre 11 pagine `ja/dokkai/` e nell'indice.
- **Nove pagine + 54 PDF**, tessera aggiunta ai 9 indici delle letture nella sezione Scienza e 9 voci nella sitemap. Audit di lingua, sito, link, vocabolario e grammatica tutti verdi; `npm run check` pulito a parte i 4 file già fuori formato da prima.
- **Parità di migrazione**: 490 differenze su 1119 pagine (invariata), **36 pagine «extra in dist»** — le 27 di prima più le 9 di questa lettura. La soglia in `AGENTS.md` è stata aggiornata.

## 2026-09-11 - Lezione «Congiuntivo imperfetto» (C1) ampliata: irregolari, periodo ipotetico e 30 esercizi

- **La richiesta di Martin**: la scheda non aveva verbi irregolari, nemmeno _avere_, e 10 esercizi erano pochi. Portarla a 30, di cui 20 sul periodo ipotetico con _se_, più completa ma sempre facile da capire.
- **Contenuto nuovo, nelle 9 lingue**: come si forma (una regola sola, tabella con la colonna delle desinenze, _io_ e _tu_ uguali, _voi_ con una sola _s_); i veri irregolari _essere_, _dare_, _stare_ più _avere_, regolare ma indispensabile; il «trucco dell'imperfetto» per _fare_, _dire_, _bere_ e i verbi in _-urre_, _-orre_, _-arre_ (_facevo_ → _facessi_); i verbi irregolari al presente ma regolari qui; la confusione con il passato remoto (_dessi/diedi_, _dicessi/dissi_); otto usi invece di quattro (_come se_, _magari_, _benché/affinché/prima che/senza che_); concordanza con il congiuntivo trapassato per l'azione precedente e l'infinito quando il soggetto è lo stesso; una sezione sul periodo ipotetico (i tre tipi, _se fossi in te_, la frase con _se_ in seconda posizione, il periodo misto, il colloquiale _se lo sapevo, venivo_); otto errori tipici invece di quattro.
- **30 esercizi in due parti**: 10 su forme, irregolari e concordanza; 20 con _se_, tutti sul congiuntivo imperfetto, con la conseguenza al condizionale già scritta. Nella prima versione gli esercizi 25-28 chiedevano il condizionale presente; Martin ha fatto notare che così verificavano un altro tempo e lo stesso giorno sono stati sostituiti (_servisse_, _prendessimo_, _faceste_, _potessero_). **Regola**: in una scheda su un tempo verbale ogni esercizio verifica quel tempo, anche quando la frase ne contiene altri. Il 29 (periodo misto) accetta anche _fosse stata_.
- **Gli usi sono passati da `.rule-card` a `.example`**: `generate-pdfs.py` non stampa le `.rule-card`, quindi quegli esempi mancavano dal PDF.
- **Spiegazioni tradotte di nuovo, a mano**: quelle vecchie venivano dalla traduzione automatica (in giapponese la nota diceva «if I possible», in turco «olsaydım, olsaydım») e in tutte le lingue era rimasto `Corretto:` in italiano. Etichette di tabella, prefissi e voci di navigazione vengono da `scripts/data/labels.mjs` e `table-cells.mjs`, come vuole l'audit; aggiunta la voce `Con se`. Le frasi degli esercizi hanno ora `lang="it"`.
...
- **Il difetto**: nei bottoni sotto gli esercizi (`.exercise-actions`) e nei bottoni «← / →» fra lezioni A1 erano rimaste traduzioni automatiche parola per parola. Controllo fatto confrontando, per ogni lingua, tutti i testi dei bottoni delle lezioni con la versione italiana.
- **«Ripassa essere»** (articoli indeterminativi → _essere_) era diventato «Review being», «Revisar siendo», «Examen étant», «Recenze bytí», «Przejrzyj bycie», «Varlığı gözden geçir», «Rezension sein», «存在をレビューする». Il verbo resta italiano con `<span lang="it">` (capitolo 2 di `REGOLE_LINGUE.md`): `Review essere`, `Repasar essere`, `Réviser essere`, `Zopakovat essere`, `Powtórz essere`, `essere fiilini tekrarla`, `essere wiederholen`, `essere を復習する`.
- **«Torna al percorso»** (aggettivi qualificativi) e **«Torna al percorso A1 →»** (preposizioni semplici) erano tradotti come «route/ruta/itinéraire/trasa/Route/Rota/ルート», cioè una strada. Ora: _learning path_, _itinerario_, _parcours_, _studijní plán_, _ścieżka nauki_, _öğrenme yolu_, _Lernpfad_, 学習コース.
- **Stessa etichetta ovunque**: `Volver a la gramática` (10 lezioni erano in minuscolo) e `Wróć do gramatyki` (10 erano «Wracając»). Il reset alternava due forme: ora `Začít znovu` (cs), `Neu beginnen` (de) e `Baştan başla` (tr) in tutte le 22 lezioni, le stesse di `create-a1-numbers-time-lessons.mjs` e del congiuntivo imperfetto.
- **Bottoni fra le lezioni A1** (presente indicativo, genere e numero, preposizioni semplici) allineati al titolo H1 della lezione collegata: cs «Současnost orientační» → «Přítomný čas», «Pohlaví a číslo» → «Rod a číslo»; pl «Obecny orientacyjny» → «Czas teraźniejszy», «Przedimki» → «Rodzajniki», «płeć» → «rodzaj»; tr «Mevcut gösterge» → «Şimdiki zaman», «makaleler» → «artikeller»; ja «現在の指示» → «現在形», «不定記事» → «不定冠詞», «性別と番号/性別と数» → «性と数», «単純な前置詞» → «単純前置詞».
- **Anche `translations/review-cache.json`** (42 voci): è la cache che usa `generate-localized-resources.mjs`, e senza correggerla una rigenerazione rimetterebbe le traduzioni sbagliate.
- **Verifica**: 108 frammenti in `src/html`; audit di lingua, sito, link e grammatica verdi; `npm run check` supera Astro ed ESLint, Prettier segnala solo file fuori formato non toccati da questa correzione. **Parità di migrazione invariata**: 490 differenze su 1119 pagine, 36 pagine extra.

## 2026-09-11 - Tre lezioni A1 nuove: i numeri, l'ora, giorni mesi e date

- **La richiesta di Martin**: nel sito mancavano spiegazioni su numeri e tempo. Vanno in grammatica e non nel vocabolario, perché hanno regole e non sono oggetti che si disegnano. Una pagina unica è stata scartata: troppo lunga, mescolerebbe tre ricerche diverse («numeri in italiano», «che ore sono», «giorni della settimana») e nella sezione grammatica «tempo» farebbe pensare ai tempi verbali. Approvate tre lezioni A1 collegate in serie, semplici, con 30 esercizi ciascuna.
- **I numeri in italiano** (`grammatica/a1/numeri.html`): da 0 a 20; decine, con la vocale che cade davanti a _uno_ e _otto_ e l'accento di _-tré_; _cento_ invariabile e _mille/-mila_; _milione_ e _miliardo_ con _di_; punto per le migliaia e virgola per i decimali; ordinali (i primi dieci, _-esimo_, i casi di _tre_ e _sei_, l'accordo); usi di tutti i giorni (età con _avere_, prezzi, percentuali, decimali); otto errori tipici.
- **Che ore sono? L'ora in italiano** (`che-ore-sono.html`): _è l'una_ e _sono le due_, _mezzogiorno_ e _mezzanotte_ senza articolo; _e, meno, un quarto, mezza_ (anche _mezzo_); _a che ora_ con _alle, all', a_; orario di tutti i giorni e orario ufficiale a 24 ore, dove non si usano _quarto, mezza_ e _meno_; parti della giornata; _buongiorno, buonasera, buonanotte_. In tedesco, ceco e polacco una nota avverte che _le tre e mezza_ è 3:30 e non «halb drei», «půl třetí», «wpół do trzeciej».
- **Giorni, mesi e date** (`giorni-mesi-date.html`): minuscole; _lunedì_ contro _il lunedì_; plurali; origine dei nomi dei giorni; mesi e stagioni con _in, a, ad_ e _d'estate_; la data con articolo e numero cardinale (ordinale solo per il primo), _l'otto_ e _l'undici_, _nel 2019_, la data nelle lettere; _ieri, oggi, domani_, _stamattina_ e _stasera_, _scorso_ e _prossimo_, la frequenza; _fa_, _tra/fra_ e _da_ + presente.
- **30 esercizi per lezione, in tre parti**, con suggerimenti in italiano e forme alternative accettate (_ventun_, _mezzo_, _trenta_, _fra_). Le spiegazioni sono scritte per ogni lingua, non tradotte in automatico, con i confronti utili a chi legge: la virgola decimale, l'ordine giorno e mese, «depuis», l'ordinale nelle date tedesche, ceche e polacche.
- **Nuovo script `scripts/create-a1-numbers-time-lessons.mjs`**, descritto in `AGENTS.md` come modello per le prossime lezioni di grammatica. In `labels.mjs` 14 voci di navigazione nuove, in `table-cells.mjs` 16 intestazioni di tabella.
- **Corretto il correttore degli esercizi** (`public/assets/grammar-exercises.js`): chi scriveva «e'» al posto di «è» riceveva «Meglio scrivere: e'». Ora il consiglio mostra sempre la forma con l'accento. Vale anche per le lezioni su _essere_ e sul _passato prossimo_, che accettavano già «e'».
- **27 pagine + 27 PDF**, 27 tessere negli indici di grammatica, 27 voci nella sitemap.
- **Verifica**: audit di lingua, sito, link, vocabolario e grammatica verdi, anche su una copia pulita del repository con solo questi file, perché un'altra sessione stava correggendo in parallelo i bottoni delle lezioni; `npm run check` senza errori; generatore rilanciato senza duplicati; nel browser controllati esercizi, forme alternative, accenti, suggerimenti, punteggio e ancore sulla pagina italiana e su quella giapponese; PDF italiano e giapponese guardati. **Parità di migrazione**: 491 differenze su 1119 pagine, la nuova è l'indice italiano di grammatica (voluta); 63 pagine extra (36 + 27).

## 2026-09-21 - Lettura «Integratori per la palestra» in tutte e 9 le lingue

- **Perché.** Richiesta di Martin: una lettura sugli integratori per la palestra — creatina e compagnia — con i pro e i contro. È il naturale seguito della lettura sulle proteine del 2026-09-07 e copre le ricerche più frequenti di chi si allena («la creatina fa male ai reni», «serve la fase di carico», «i BCAA servono»).
- **Sedici fatti in tutti e cinque i livelli**, come vuole la regola del 2026-08-31: che cos'è un integratore per la legge europea (direttiva 2002/46/CE: è un alimento, nessuno deve dimostrarne l'efficacia prima di venderlo), i cinque prodotti con prove solide del consenso CIO 2018 (creatina, caffeina, nitrato, beta-alanina, bicarbonato), la creatina monoidrato a 3-5 g al giorno e la fosfocreatina dei primi dieci secondi, l'acqua dei primi 1-2 chili, i reni sani e la creatinina che sale nelle analisi, i non-responder (2-3 su 10), la caffeina a 3 mg/kg con i limiti EFSA (400 mg al giorno, 200 in una volta), l'emivita di 5 ore e il sonno perso, la beta-alanina (3-6 g per almeno 4 settimane, formicolio sopra gli 800 mg), il bicarbonato (0,2-0,3 g/kg, stomaco permettendo), le proteine in polvere come cibo comodo, i BCAA (tre amminoacidi su nove), i prodotti senza prove e la «miscela proprietaria», la contaminazione (Colonia 2004: 634 prodotti, 14,8% con ormoni non dichiarati) e il fegato (un caso su cinque negli Stati Uniti), le vitamine solo dopo le analisi, e le proporzioni fra allenamento, cibo, sonno e integratori.
- **Pro e contro davvero da tutte e due le parti**, come nella lettura sulle proteine: la scheda **ESEMPI** racconta i casi di contaminazione (Kicker Vencill 2003 e il risarcimento del 2005, Jessica Hardy 2008, l'efedra vietata nel 2004, il DMAA vietato dalla FDA nel 2013) **e** i due casi opposti, creatina e caffeina, che hanno superato ogni verifica. La conclusione è esplicita: i due estremi esistono entrambi e non si distinguono dal barattolo.
- **Struttura identica al modello delle proteine**: cinque livelli, tre approfondimenti con parole e domande (la creatina dal principio; il giorno della gara; leggere un'etichetta e un prezzo), tre schede di solo testo — **ESEMPI**, **MITI** (dieci miti smontati) e **SPESA** (costo per dose efficace e sei regole prima di comprare) — e due **LAB** di lingua nuovi rispetto a quelli delle proteine: il lessico della palestra (_assumere_/_prendere_/_sciogliere_, _dose_ e _posologia_, _esercizio_/_ripetizione_/_serie_/_scheda_, _efficace_ contro _efficiente_, _effetto collaterale_ contro _controindicazione_) e il modo di dire il tempo (_prima di_ + infinito, _dopo_ + infinito passato, _fra_/_prima_/_entro_, e la terna _per_ / _in_ / _da_ quattro settimane).
- **Tredici fonti in fondo alla pagina**, con link: consenso CIO 2018, le posizioni ISSN su creatina (2017), beta-alanina (2015), caffeina (2021) e bicarbonato (2021), il parere EFSA sulla caffeina (2015), la direttiva 2002/46/CE, Geyer 2004, Wolfe 2017, Morton 2018, Jones 2014, Navarro 2014 e il divieto FDA del DMAA.
- **Nuovo script `scripts/create-integratori-reading.mjs`**, sullo stampo di `create-proteine-reading.mjs`: testo italiano in `scripts/data/integratori-reading-it.mjs`, cornice localizzata in `scripts/data/integratori-reading-i18n.mjs`, e metadati, call to action e tessera «Scienza» clonati **dalla lettura sulle proteine**, che ha già la stessa struttura di schede ed è revisionata in tutte e 9 le lingue.
- **Immagine di testata con `google/gemini-3-pro-image`** ($0,1349), come da decisione del 2026-09-03: misurino di polvere, shaker d'acciaio, succo di barbabietola, caffè, compresse in una ciotola di ceramica, manubrio e asciugamano su un tavolo di legno. **Nessuna carne, nessun pesce, nessun uovo** (decisione del 2026-09-07) e, in più, **nessuna etichetta e nessun marchio riconoscibile**: la lettura spiega che le promesse in copertina non valgono niente, una confezione di marca sembrerebbe pubblicità. Prompt e vincoli in `docs/prompt-immagine-integratori.md`.
- **Nove pagine + 54 PDF**, tessera aggiunta ai 9 indici delle letture nella sezione Scienza e 9 voci nella sitemap. Audit di lingua, sito, link, vocabolario e grammatica tutti verdi; `npm run check` pulito a parte i 4 file già fuori formato da prima.
- **Parità di migrazione**: 491 differenze su 1119 pagine (invariata), **72 pagine «extra in dist»** — le 63 di prima più le 9 di questa lettura. La soglia in `AGENTS.md` è stata aggiornata.

## 2026-09-21 - Il menu del telefono mancava su 54 pagine: `script.js` ora lo mette il layout

- **Come è saltato fuori.** Martin ha notato che nella lettura nuova sugli integratori, su schermo stretto, l'header mostra solo il logo e la bandiera: niente menu. Il confronto con le altre pagine ha mostrato che il difetto non era della pagina nuova — la lettura sulle proteine, da cui è stata clonata, si comporta allo stesso modo.
- **La causa.** Sotto i 900 pixel `public/styles.css` nasconde `nav` e mostra al suo posto il bottone con le tre righe; quel bottone però non sta nell'HTML, lo costruisce `public/script.js`. Il riferimento allo script arrivava dall'elenco `bodyScripts` di ogni pagina, ereditato dal sito storico, e in sei letture era vuoto: **DNA, latte materno, mafia, insetto con gli ingranaggi, proteine e integratori**, cioè 54 pagine. Senza script niente bottone, e con `nav` nascosto dal CSS il menu spariva e basta. Sul desktop non si vedeva niente, perché sopra i 900 pixel `nav` è visibile da solo.
- **La correzione sta in `src/layouts/SiteLayout.astro`**, non nelle singole pagine: il layout aggiunge `script.js` quando `bodyScripts` non lo contiene già. Le pagine che lo dichiaravano lo tengono **identico a prima**, versione e percorso compresi: duplicarlo farebbe scattare l'audit dei link, cambiarlo farebbe salire le differenze di parità. Dopo la correzione tutte le pagine con header hanno lo script esattamente una volta; restano fuori solo `404.html` e il file di verifica di Google, che l'header non ce l'hanno.
- **Regola che ne esce**: quello che deve valere su tutte le pagine si mette nel layout, non nei metadati della pagina. `bodyScripts` resta per gli script di una pagina sola (glossario, esercizi, riquadri delle risposte).
- **Verifica**: menu aperto e chiuso a 375 pixel sulla lettura italiana e su quella giapponese, home e lezione di grammatica ricontrollate (script una volta sola, 30 esercizi funzionanti). Audit di lingua, sito, link, vocabolario e grammatica verdi; nessun file caricato due volte; **parità di migrazione invariata**: 491 differenze su 1119 pagine, 72 pagine extra.

## 2026-09-23 - Favola «La formica wow», solo in italiano e solo a livello A1

- **Perché è un'eccezione.** Martin ha scritto una favola originale (una formica che ammira un'aquila invece di lavorare, viene derisa e chiamata «wow», poi vola davvero con l'aquila) e ha chiesto di pubblicarla come esperimento: solo in italiano, solo a livello A1, senza gli altri quattro livelli e senza traduzione nelle altre 8 lingue. È una rottura deliberata delle due regole del 2026-08-25 e di sempre («ogni pagina esiste in italiano più 8 lingue», «i livelli di una lettura raccontano le stesse cose»): Martin ne è stato avvisato esplicitamente perché il controllo `audit-site.mjs --strict` blocca l'intero deploy se una risorsa sotto `letture/`, `favole/` o `grammatica/` non ha le 8 traduzioni. Ha scelto di registrare un'eccezione nota nello script piuttosto che tradurre subito o saltare i controlli.
- **La pagina**: `favole/la-formica-wow.html`, un solo `story-card` (livello A1, solo presente), senza `level-nav` e senza bottoni PDF (nessun PDF generato per questa pagina). Due domande di comprensione con `<textarea>`, come da convenzione. Immagine di testata fornita da Martin (aquila che porta due formiche in volo su un paesaggio toscano), ritagliata a 16:9 e salvata come `story-formica-wow.webp` / `-card.webp`.
- **`scripts/audit-site.mjs`**: nuovo insieme `PARTIAL_RESOURCES` con `favole/la-formica-wow.html`. La risorsa è esclusa dal controllo di `hreflang` reciproco, dal controllo dei PDF per livello e dal conteggio `expectedPdfCount`, ma resta soggetta a tutto il resto (deve comunque comparire nell'indice italiano di `favole/` e nella sezione Favole di `letture/`, cosa già fatta). Se in futuro la favola verrà tradotta o completata con gli altri livelli, va tolta da `PARTIAL_RESOURCES`.
- **Hreflang della pagina**: solo `it` e `x-default`, entrambi verso se stessa; il selettore di lingua in pagina mostra solo l'italiano, per non creare link morti verso traduzioni che non esistono.
- **Verifica**: `audit-language-mix`, `npm run build`, `audit-site --strict`, `audit-links --strict`, `audit:vocabulary`, `audit:grammar-seo` tutti verdi; `npm run check` pulito (stessi 4 file già fuori formato da prima). Pagina controllata nel browser: testo, parole utili, domande e tessere negli indici di `favole/` e `letture/` corrette.
- **Rinominata lo stesso giorno**: da «La formica wow» a «La formichina Wow» (titolo, H1, tessere negli indici) e URL da `favole/la-formica-wow.html` a `favole/la-formichina-wow.html`. Siccome l'URL vecchio era già pubblicato, resta come redirect `noindex,follow` in `public/favole/la-formica-wow.html` (stesso schema dei redirect di grammatica). `PARTIAL_RESOURCES` aggiornato al nuovo percorso. **Nuova regola in `audit-site.mjs`**: `collectResources`/`collectMenuResources` ora escludono qualunque file con `http-equiv="refresh"` (uno stub di redirect non è una risorsa educativa e non deve avere traduzioni o PDF propri) — utile per ogni futuro redirect creato direttamente sotto `letture/`, `favole/` o `grammatica/` (radice italiana), a differenza dei redirect nelle sottocartelle di lingua che l'audit non ha mai guardato.

## 2026-09-24 - Vocabolario: «Gli animali» e «Le caratteristiche degli animali»

- **La richiesta di Martin.** Una lezione di vocabolario sugli animali (fino a 50, mammiferi, rettili, pesci ecc.) e una seconda scheda con le caratteristiche (pigro, laborioso, vanitoso, coraggioso, amichevole, solitario…, fino a 50 aggettivi), con un esercizio per associare le caratteristiche agli animali, anche con le negazioni («quali animali non sono…»). Per gli animali immagini simpatiche ma realistiche; foto di contenuto con il modello economico, testata di qualità.
- **Gli animali** (`vocabolario/animali.html`): 50 parole — 26 mammiferi, 9 uccelli, 5 rettili, 1 anfibio, 4 pesci, 5 fra insetti, ragni e chiocciole — con tre frasi d'esempio, pronuncia, «Riconosci la parola» (50) e 12 frasi da tradurre, come le altre lezioni. Foto con `gpt-image-1-mini` a qualità bassa ($0,0025 l'una), in stile fotografico «cucciolo simpatico ma realistico»; nel prompt «foto a figura intera con margine bianco», dopo che i primi due tentativi (pavone e cigno) erano venuti ritagliati o a tutto campo. Nessuna carne e nessuna preda in scena; l'orso goloso tiene una fragola, non un vasetto di miele.
- **Le caratteristiche degli animali** (`vocabolario/caratteristiche-animali.html`): 50 aggettivi con le due forme (pigro / pigra), una riga di spiegazione nella lingua del visitatore (in italiano: una definizione), tre frasi d'esempio, una foto di un animale che mostra la caratteristica e 12 frasi da tradurre. **Senza «Riconosci la parola»**: un aggettivo non si riconosce da una foto (un bradipo può essere «pigro», «sonnolento» o «tranquillo»). Al suo posto due esercizi con trascinamento, dove **basta trovare almeno un animale** (regola di Martin):
  - **«Quale animale è così?»**: 50 righe in 5 serie da 10. Ogni serie ha la sua barra di 14 animali, sempre in fondo allo schermo mentre si scorre, con almeno una risposta giusta per ogni aggettivo.
  - **«Quale animale non è così?»**: 20 aggettivi in 2 serie, scelti fra quelli con caratteristiche verificabili (veloce/lento, grande/piccolo, pesante/leggero, velenoso, domestico/selvaggio…). Giusto è ogni animale che non compare fra quelli associabili; nella barra ci sono anche gli animali che la caratteristica ce l'hanno, come trappole.
  - Si risponde trascinando, oppure toccando l'animale e poi la riga (telefono), oppure da tastiera. Un errore si vede per un attimo e sparisce; un pulsante «Suggerimento» fa pulsare un animale giusto; «Ricomincia» azzera; i progressi restano sul dispositivo.
- **Stereotipi dichiarati.** Le associazioni sono in gran parte quelle dei modi di dire italiani («furbo come una volpe», «testardo come un mulo»): la pagina lo scrive esplicitamente in una nota sotto le schede, in tutte le lingue, per non farle passare per fatti scientifici.
- **Testate**: due illustrazioni morbide a guazzo (nessun contorno nero) con `google/gemini-3-pro-image`, $0,13 l'una, nello stesso stile della testata del mare. Il primo tentativo è andato male per un mio errore: ho passato al generatore un frammento sbagliato del file dei prompt, e il modello ha inventato un collage fotografico con un titolo scritto. Corretto e rifatto; il costo del tentativo sbagliato è di un'immagine.
- **Lingue**: 9 lingue per entrambe le lezioni (18 pagine), con titoli, descrizioni, testi degli esercizi e alt tradotti per lingua; le forme italiane dentro i testi tradotti stanno in `<em lang="it">`. Due schede nuove in ognuno dei 9 indici del vocabolario, 18 voci nella sitemap. Nuovo script `create-animals-vocabulary.mjs` (idempotente), `convert-vocabulary-images.mjs` ora accetta `--subdir`, `generate-image.mjs` accetta `--aspect-ratio` e `--no-card`.
- **Verifica**: audit di lingua, sito, link, vocabolario (ora anche righe e risposte giuste degli esercizi con trascinamento) e grammatica verdi; nel browser provati trascinamento, risposta sbagliata, tocco e riga, suggerimento, indice e pagina inglese e giapponese. **Parità di migrazione**: 491 differenze su 1119 pagine (invariata), **92 pagine extra** (72 + 2 della favola + 18 di queste lezioni).
