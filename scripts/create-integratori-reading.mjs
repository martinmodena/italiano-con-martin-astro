#!/usr/bin/env node
// Crea la lettura «Integratori per la palestra: che cosa funziona e che cosa
// no» in tutte e 9 le lingue, seguendo il modello di
// scripts/create-proteine-reading.mjs:
//
...
//
// Il testo di studio (italiano) sta in scripts/data/integratori-reading-it.mjs,
// la cornice localizzata in scripts/data/integratori-reading-i18n.mjs.
//
// Le pagine .astro, la call to action e la tessera non si scrivono a mano: si
// clonano dalla lettura «Proteine: quante ne servono davvero», gia' tradotta e
// revisionata in tutte e 9 le lingue e con la stessa struttura di schede
// (approfondimenti, ESEMPI, MITI, SPESA, LAB). Cosi' nav, footer, icone ed
// etichette restano quelle gia' corrette in ogni lingua.
//
// Uso:  node scripts/create-integratori-reading.mjs [--dry-run]

import { readFileSync, writeFileSync, mkdirSync } from 'node:fs';
import path from 'node:path';
import { LANGS, i18n } from './data/integratori-reading-i18n.mjs';
import { levels, insights, examples, myths, practice, labs, sources } from './data/integratori-reading-it.mjs';

const ROOT = process.cwd();
const DRY = process.argv.includes('--dry-run');
const IMAGE = 'reading-integratori';

// Risorsa da cui si clona la cornice gia' tradotta: la lettura sulle proteine
// (metadati, call to action e tessera «Scienza»).
const SOURCE = {
  it: 'letture/proteine-quante-ne-servono',
  en: 'en/readings/how-much-protein-do-we-need',
  es: 'es/lecturas/cuanta-proteina-necesitamos',
  fr: 'fr/lectures/combien-de-proteines-nous-faut-il',
  cs: 'cs/cteni/kolik-bilkovin-potrebujeme',
  pl: 'pl/czytanki/ile-bialka-potrzebujemy',
  tr: 'tr/okumalar/ne-kadar-proteine-ihtiyacimiz-var',
  de: 'de/lesetexte/wie-viel-eiweiss-brauchen-wir',
  ja: 'ja/dokkai/タンパク質はどれだけ必要か',
};
const SOURCE_SLUG = Object.fromEntries(LANGS.map((l) => [l, SOURCE[l].split('/').pop()]));
const SOURCE_IMAGE = 'reading-proteine';
...
// Scheda di solo testo (esempi, miti, spesa, LAB): titolo e occhiello tradotti,
// corpo in italiano perche' e' materiale di studio.
...
function buildFragment(lang) {
  ...
  // La call to action finale e' identica a quella della lettura sulle
  // proteine: stessa lingua, stessi link, gia' revisionata.
  const source = readFileSync(path.join(ROOT, 'src/html', `${SOURCE[lang]}.html`), 'utf8');
...
function buildAstro(lang) {
  ...
  const src = readFileSync(path.join(ROOT, 'src/pages', `${SOURCE[lang]}.html.astro`), 'utf8');
  ...
  let out = src;
  for (const l of LANGS) out = out.split(`/${SOURCE[l]}.html`).join(`/${routeOf(l)}.html`);
  out = out.split(`~/html/${SOURCE[lang]}.html`).join(`~/html/${route}.html`);
  ...
  meta.og = meta.og.map(([k, v]) => {
    if (k === 'og:title') return [k, lang === 'it' ? `${t.cardTitle} - una lettura graduata A1-C1` : t.metaTitle];
...
function updateIndex(lang) {
  ...
  const model = SOURCE_SLUG[lang];
  const re = new RegExp(
    `<a class="story-tile" href="[^"]*${model.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\.html"[\\s\\S]*?</a>`
  );
  const match = html.match(re);
  if (!match) throw new Error(`tessera delle proteine non trovata in ${rel}`);

  const tile = match[0]
    .split(model)
    .join(t.slug)
    .replace(`${SOURCE_IMAGE}-card`, `${IMAGE}-card`)
