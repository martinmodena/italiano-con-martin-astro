...
- **Le lezioni «Gli animali» e «Le caratteristiche degli animali» (2026-09-24)** nascono da `scripts/create-animals-vocabulary.mjs`, che segue lo stampo del mare (parte dalla lezione della cucina già tradotta) e le costruisce tutte e due. Dati in `scripts/data/animals-vocabulary.mjs` (50 animali), `traits-vocabulary.mjs` (50 aggettivi, con gli animali che si possono associare a ciascuno in `matches` e quelli che chiaramente non lo sono in `never`) e `animals-pages.mjs` (titoli, descrizioni, testi degli esercizi in 9 lingue). Le foto delle parole stanno in sottocartelle (`public/assets/vocabolario/animali/`, `.../caratteristiche/`): si generano con `scripts/generate-animal-images.mjs` (stile «simpatico ma realistico», modello economico), si ripuliscono con `remove-white-background.py` e si convertono con `convert-vocabulary-images.mjs <cartella> --subdir <nome>`. Le testate sono illustrazioni di qualità (`generate-image.mjs --aspect-ratio 3:2 --no-card`); prompt in `docs/prompt-immagini-animali.md`.

  **La lezione sulle caratteristiche non ha «Riconosci la parola»** (un aggettivo non si riconosce da una foto): al suo posto ha due esercizi con trascinamento — «quale animale è così?» (50 righe, in 5 serie) e «quale animale non è così?» (20 righe, in 2 serie) — dove basta trovare **almeno un animale**. Comportamento in `public/assets/match.js` (trascinamento con eventi pointer, oppure tocco sull'animale e poi sulla riga, oppure tastiera), stili in `public/assets/match.css`; i messaggi arrivano dagli attributi `data-msg-*` della sezione, quindi il JavaScript non ha traduzioni. Se si tocca uno dei due file va alzato `MATCH_ASSET_VERSION` nello script. `audit-vocabulary.mjs` controlla che ogni riga abbia almeno una risposta giusta fra gli animali della sua barra. Le associazioni sono in gran parte stereotipi dei modi di dire italiani («furbo come una volpe»): la pagina lo dice, e `matches` è generoso di proposito.
...
- **Rete di sicurezza**: `node scripts/migrate/verify-parity.mjs --ignore-intended-fixes` confronta la build con il sito storico neutralizzando le correzioni volute. Dal 2026-09-01 la base di confronto è **490 differenze su 1119 pagine**, tutte volute: le 36 precedenti (18 indici senza elenco testuale, 9 home riscritte, 9 pagine «Chi siamo» e la home inglese con la tariffa di 12 €) più le 469 pagine localizzate riparate secondo `REGOLE_LINGUE.md`. Alle 485 del 2026-08-25 si aggiungono le 9 pagine della lettura «Il latte materno», riscritta a mano il 2026-08-31. Dal 2026-09-01 si aggiunge `vocabolario/cibo.html`, la pagina italiana della lezione «Il cibo» ampliata a 16 parole (le 8 localizzate erano già fra le 469). Dal 2026-09-11 si aggiunge `grammatica/index.html`, l'indice italiano con le tre lezioni A1 su numeri, ore e date: la base diventa **491**. Se il numero sale oltre 491, è cambiato qualcosa che non era previsto: va capito prima di pubblicare. Le pagine **«extra in dist»** sono invece quelle nate dopo la migrazione e assenti da `legacy-html/`: dal 2026-09-24 sono **92**, cioè otto risorse per nove lingue più le due lezioni di vocabolario sugli animali (18 pagine) e le due pagine della favola «La formichina Wow» (quella con il redirect e quella nuova) — la lettura «La storia della mafia in Italia» (2026-09-02), la lezione di vocabolario «Il mare» (2026-09-03, non registrata allora), la lettura «L'insetto con gli ingranaggi» (2026-09-04), la lettura «Proteine: quante ne servono davvero» (2026-09-07), le lezioni di grammatica A1 «I numeri in italiano», «Che ore sono?» e «Giorni, mesi e date» (2026-09-11) la lettura «Integratori per la palestra» (2026-09-21) e le lezioni di vocabolario «Gli animali» e «Le caratteristiche degli animali» (2026-09-24).
